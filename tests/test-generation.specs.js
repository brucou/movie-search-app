...
import {
  computeTimesCircledOn, generateTestSequences, testFsm, create_state_machine, INIT_EVENT, INIT_STATE, NO_OUTPUT, NO_STATE_UPDATE
...
const { SEARCH_ERROR_MOVIE_RECEIVED, USER_NAVIGATED_TO_APP, QUERY_CHANGED, MOVIE_DETAILS_DESELECTED, MOVIE_SELECTED, SEARCH_ERROR_RECEIVED, SEARCH_RESULTS_MOVIE_RECEIVED, SEARCH_RESULTS_RECEIVED } = events;
...
// Here we generate input sequences by generating paths from the initial state to the `MOVIE_SELECTION` state
// To have sufficient coverage but remain under a manageable number of tests, we traverse a max of three times any path.
// Once the input sequences are generated, we run them against the machine to get the actual outputs
// We compute the expected outputs and compare both
// Note that our oracle works under the strict hypothesis that all input sequences are well-formed (no illegal
// sequence of events)
// Conclusion of the tests :
// - 46 tests generated
// - 11 pass
// - the failing tests are exactly those which involve concurrent requests (which arrive out of order)
QUnit.test("With search concurrency", function exec_test(assert) {
  function generateInputSequences(){
  ...
  const inputSequences = generateInputSequences();
  ...
  testFsm({testAPI, fsmFactorySpecs, inputSequences, oracle, format});
});
